...
public class Rank : BaseUI
{
    ...
    protected override void Awake()
    {
        ...
        {
            coin_rankButton.transform.localPosition -= new Vector3(0, 100);
            ticket_rankButton.transform.localPosition -= new Vector3(0, 100);
            top_lineRect.transform.localPosition -= new Vector3(0, 100);

            coin_rank_topRect.localPosition -= new Vector3(0, 100);
            coin_rank_topRect.sizeDelta += new Vector2(0, Screen.height - 1920 - 200);
            coin_rank_topRect.GetComponentInChildren<ScrollRect>().normalizedPosition = Vector2.one;
            ticket_rankRect.localPosition -= new Vector3(0, 100);
            ticket_rankRect.sizeDelta += new Vector2(0, Screen.height - 1920 - 200);
    ...
    private void InitCoinRanks()
    {
        ...
        List<RankInfo> allGoldRankInfo = Save.data.rank_data.gold_rank;
        ...
        for(int i = 0; i < allRankCount; i++)
        {
            ...
            RankInfo rankInfo = allGoldRankInfo[i];
        ...
        RankInfo selfRankInfo = Save.data.rank_data.self_gold_info;
    ...
    private void InitTicketRanks()
    {
        ...
        List<RankInfo> allTicketRankInfo = Save.data.rank_data.tickets_rank;
        ...
        for(int i = 0; i < allRankCount; i++)
        {
            ...
            RankInfo rankInfo = allTicketRankInfo[i];
    ...
    {
        Server.Instance.RequestData(Server.Server_RequestType.RankData, OnRequestDataCallback, null);
    }
